import $ from 'jquery';
import { myCanvas } from './canvasData';
const canvas = document.getElementById('canvasDiv');
const context = canvas.getContext("2d");
const buttonPredict = document.createElement('button');
const buttonClear = document.createElement('button');
const buttonBeginTrain = document.createElement('button');
const result = document.getElementById('result');
const statusTrain = document.getElementById('statusTrain');
buttonPredict.innerHTML = 'prediction';
buttonClear.innerHTML = 'clear';
buttonBeginTrain.innerHTML = 'Begin Train';
buttonBeginTrain.style.width = '100px';
buttonBeginTrain.style.height = '50px';
document.getElementById('action').appendChild(buttonPredict);
document.getElementById('action').appendChild(buttonClear);
document.getElementById('status').appendChild(buttonBeginTrain);

buttonPredict.addEventListener("click", async function () {
  await loadCanvas();
  showPredictionsOnCanvas();
  submitted = true;
});
...
buttonBeginTrain.addEventListener("click", function () {
  buttonBeginTrain.innerHTML = 'Loading ....';
  mnist();
});
...
$('#canvasDiv').mousedown(function (e) {
  paint = true;
  addClick(e.pageX - this.offsetLeft - 2, e.pageY - this.offsetTop);
  redraw();
  if (submitted) {
    result.innerHTML = '';
    clickX = [];
...
function redraw() {
  context.clearRect(0, 0, context.canvas.width, context.canvas.height);
  context.strokeStyle = "rgb(223, 75, 38)";  
...
async function showPredictionsOnCanvas() {
  const testExamples = 1;
  const batch = dataCanvas.nextTestBatch(testExamples);
  tf.tidy(() => {
...
async function loadCanvas() {
  ...
  const imgRequest = new Promise((resolve, reject) => {
    img.onload = async () => {     
      ...
      resizeImg = canvas.toDataURL();
      dataCanvas = new myCanvas();
      await dataCanvas.load(resizeImg);
      resolve();
    };
...
async function mnist() {
  await load();
  await train();
  statusTrain.innerHTML = 'trained';
  statusTrain.style.color = 'green';
  showPredictions();
}
//mnist();
